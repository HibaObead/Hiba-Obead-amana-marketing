      ...
      <div className="p-6">
        <div className="overflow-auto rounded-lg border border-gray-700" style={{ maxHeight }}>
          <table className="w-full">
            <thead className="bg-gray-900 sticky top-0">
              <tr>
                {showIndex && <th className="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">#</th>}
                {columns.map((column) => (
                  <th
                    key={column.key}
            ...
            <tbody className="bg-gray-800 divide-y divide-gray-700">
              {sortedData.map((row, index) => (
                <tr key={index} className="hover:bg-gray-750 transition-colors duration-150">
                  {showIndex && <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-300">{index + 1}</td>}
                  {columns.map((column) => (
                    <td
                      key={column.key}
                      className={`px-4 py-4 whitespace-nowrap text-sm text-gray-300 ${column.align === 'center' ? 'text-center' :
                        column.align === 'right' ? 'text-right' : 'text-left'}`}
                    >
